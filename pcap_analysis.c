...
/* TCP 的连接释放 */
#define FIN_WAIT_1  0x04    // client send FIN  客户端A发送连接释放报文，FIN=1
...
typedef struct _net_link_header
{
    ...
    int count_upl; // 数据包总上传
    int count_downl; // 数据包总下载
    p_net_link link; // 第一个数据包地址

    int DNS_count_conn;//DNS连接个数
    int HTTP_count_conn;
    int HTTPS_count_conn;
    int DNS_count_upl;//为了DNS类型的流量计算...
    int DNS_count_downl;
    int HTTP_count_upl;//为了HTTP类型的流量计算...
    int HTTP_count_downl;
    int HTTPS_count_upl;//为了HTTPS类型的流量计算...
    int HTTPS_count_downl;

    int DNS_count_upl_pkt; // DNS数据包总上传数据量
    int DNS_count_downl_pkt; // DNS数据包总下载数据量
    int HTTP_count_upl_pkt; // HTTP数据包总上传数据量
    int HTTP_count_downl_pkt; // HTTP数据包总下载数据量
    int HTTPS_count_upl_pkt; // HTTPS数据包总上传数据量
    int HTTPS_count_downl_pkt; // HTTPS数据包总下载数据量
...
char *longTime(long ltime)
{
    ...
    t = ltime;
    p = localtime(&t);//localtime是把从1970-1-1零点零分到当前时间系统所偏移的秒数时间转换为本地时间
...
{
    head->count_conn        = 0;// 连接个数
    head->count_upl_pkt     = 0;// 数据包总上传数据量
    head->count_downl_pkt   = 0;// 数据包总下载数据量
    head->count_upl         = 0;// 数据包总上传
    head->count_downl       = 0;// 数据包总下载
    head->link              = NULL;// 第一个数据包地址

    head->DNS_count_conn;
    head->HTTP_count_conn;
    head->HTTPS_count_conn;

    head->DNS_count_upl;
    head->DNS_count_downl;
    head->HTTP_count_upl;
    head->HTTP_count_downl;
    head->HTTPS_count_upl;
    head->HTTPS_count_downl;

    head->DNS_count_upl_pkt;
    head->DNS_count_downl_pkt;
    head->HTTP_count_upl_pkt;
    head->HTTP_count_downl_pkt;
    head->HTTPS_count_upl_pkt;
    head->HTTPS_count_downl_pkt;
...
{
    if( head->link == NULL ){ return;}//数据包地址
    
    net_link_node *pTemp1 = NULL;
    net_link_node *pTemp2 = NULL;
    
    pTemp1 = head->link;//数据包地址
    pTemp2 = pTemp1->next;
    while( pTemp2 != NULL )
    {
        free(pTemp1);
...
/* TCP 数据包流量分析，todo增加协议类型的分类计算,todo增加ＩＣＭＰ */
void parse_flowLink_TCP(FILE *fOutput)
{
    fprintf(fOutput, "TCP连接个数：\t%d\n", FLowLink_TCP->count_conn);
    fprintf(fOutput, "TCP数据包个数：\t%d\n", FLowLink_TCP->count_upl_pkt + FLowLink_TCP->count_downl_pkt);//上传加下载
    fprintf(fOutput, "TCP数据总流量：\t%d bytes\n", FLowLink_TCP->count_upl + FLowLink_TCP->count_downl);//上传加下载
    fprintf(fOutput, "TCP数据包总上传：\t%d bytes\n", FLowLink_TCP->count_upl);
    fprintf(fOutput, "TCP数据包总下载：\t%d bytes\n", FLowLink_TCP->count_downl);
    fprintf(fOutput, "--------------------------------------------------------\n");
...
void add_to_flowLink(net_link_header *head, const net_link_node *theNode)
{
    ...
    head->count_conn ++;                                //连接个数
    head->count_upl_pkt     += newNode->nln_upl_pkt;    // 数据包总上传数据量
...
/*在以太网中，规定最小的数据包为64个字节，如果数据包不足64字节，由网卡填充*/
...
int main(int argc, char **argv)
{
    ...
    /* 读取离线存储数据包文件 */
    handle = pcap_open_offline("packet.data",errbuf);
    printf("开始分析\n");
    ...
    init_flowLink(FLowLink_TCP);//初始化流量链表
    init_flowLink(FLowLink_UDP);//初始化流量链表
    
    pcap_loop(handle, -1, pcapAnalysis, (u_char *)&cycle);

    printf("\n****************************\nDNS服务连接个数：\t%d\n", FLowLink_TCP->DNS_count_conn+FLowLink_UDP->DNS_count_conn);
    printf("\n基于TCP/IP通信应用层协议HTTP连接个数：\t%d\n", FLowLink_TCP->HTTP_count_conn);
    printf("\n超文本传输安全协议HTTPS连接个数：\t%d\n", FLowLink_TCP->HTTPS_count_conn);
    printf( "\n所有捕获的DNS服务数据流量：\t%d bytes\n", FLowLink_TCP->DNS_count_upl + FLowLink_TCP->DNS_count_downl+FLowLink_UDP->DNS_count_upl + FLowLink_UDP->DNS_count_downl);
    printf( "\n所有捕获的HTTP类型数据流量：\t%d bytes\n", FLowLink_TCP->HTTP_count_upl + FLowLink_TCP->HTTP_count_downl);
    printf( "\n所有捕获的HTTPS数据流量：\t%d bytes\n", FLowLink_TCP->HTTPS_count_upl + FLowLink_TCP->HTTPS_count_downl);
    printf( "\n所有捕获的DNS数据包个数：\t%d \n", FLowLink_TCP->DNS_count_upl_pkt + FLowLink_TCP->DNS_count_downl_pkt+FLowLink_UDP->DNS_count_upl_pkt + FLowLink_UDP->DNS_count_downl_pkt);
    printf( "\n所有捕获的HTTP数据包个数：\t%d \n", FLowLink_TCP->HTTP_count_upl_pkt+ FLowLink_TCP->HTTP_count_downl_pkt);
    printf( "\n所有捕获的HTTPS数据包个数：\t%d \n", FLowLink_TCP->HTTPS_count_upl_pkt+ FLowLink_TCP->HTTPS_count_downl_pkt);
...
void  pcapAnalysis(u_char *userarg, const struct pcap_pkthdr *pkthdr, const u_char *packet)
{
    ...
    FILE *fOutput;
    fOutput = fopen(file_output, "a+");//追加
    ...
    if((tstamp_now - tstamp_offset) >= *cycle) {
        ...
        fprintf(fOutput, " --> %s\n", longTime(tstamp_offset + *cycle));//周期的时间段
        
        /* 统计 UDP 数据包数量，数据量大小 */
        fprintf(fOutput, "--------------------------------------------------------\n");
        clear_hashTable();
        parse_flowLink_UDP(fOutput);//UDP 数据包流量分析 
        init_flowLink(FLowLink_UDP);//如果要统计所有捕获的数据包的DNS、HTTP、HTTPS的流量就把这里的两个初始化语句暂时注释
        
        /* 统计 TCP 数据包数量，数据量大小 */
        fprintf(fOutput, "--------------------------------------------------------\n");
        parse_flowLink_TCP(fOutput);
        init_flowLink(FLowLink_TCP);//上文指和这句
        fprintf(fOutput, "\n");
        /* 下一个分析周期开始时间 */
    ...
    if(ntohs(eptr->ether_type) == ETHERTYPE_IP) { // 判断是否为 IP 数据包
        if(ipptr->ip_p == IPPROTO_TCP || ipptr->ip_p == IPPROTO_UDP) { // 判断是否为 TCP UDP 数据包
            ...
            }else if(ipptr->ip_p==IPPROTO_ICMP){
                //CurSet->sport=ntohs(icmp6_pptr->ih_sport)
            }
    ...
    if(ipptr->ip_p == IPPROTO_TCP) {
        ...
        //FIN
        fprintf(fOutput, "ACK=%d     ",(tcpptr->th_ack&0x1000) != 0);
        fprintf(fOutput, "seq=%d     \n",(tcpptr->th_seq&0x1000) != 0);
        //SYN
        fprintf(fOutput, "校验和：%d\n", ntohs(tcpptr->th_sum));
        fprintf(fOutput, "紧急：%d\n", ntohs(tcpptr->th_urp));
        if((ntohs(tcpptr->th_dport)==53)||(ntohs(tcpptr->th_sport)==53))
        {
            fprintf(fOutput, "              ----类型：DNS服务----                 \n\n");
            printf("\n第%d个网络数据包类型：（TCP）DNS服务\n", (id-1));
            FLowLink_TCP->DNS_count_conn++;
            FLowLink_TCP->DNS_count_upl         +=FLowLink_TCP->count_upl;
            FLowLink_TCP->DNS_count_downl       +=FLowLink_TCP->count_downl;
            
            FLowLink_TCP->DNS_count_upl_pkt       +=FLowLink_TCP->count_upl_pkt;
            FLowLink_TCP->DNS_count_downl_pkt       +=FLowLink_TCP->count_downl_pkt;
        }
        if((ntohs(tcpptr->th_dport)==80)||(ntohs(tcpptr->th_sport)==80))
        {
            fprintf(fOutput, "              ----类型：HTTP应用层协议----                 \n\n");
            printf("\n第%d个网络数据包类型：基于TCP/IP通信应用层协议HTTP\n", (id-1));
            FLowLink_TCP->HTTP_count_conn++;
            
            FLowLink_TCP->HTTP_count_upl+=FLowLink_TCP->count_upl;
            FLowLink_TCP->HTTP_count_downl       +=FLowLink_TCP->count_downl;

            FLowLink_TCP->HTTP_count_upl_pkt+=FLowLink_TCP->count_upl_pkt;
            FLowLink_TCP->HTTP_count_downl_pkt      +=FLowLink_TCP->count_downl_pkt;
            
        }
        if((ntohs(tcpptr->th_dport)==443)||(ntohs(tcpptr->th_sport)==443))
        {
            fprintf(fOutput, "              ----类型：超文本传输安全协议HTTPS----                 \n\n");
            printf("\n第%d个网络数据包类型：超文本传输安全协议HTTPS\n", (id-1));
            FLowLink_TCP->HTTPS_count_conn++;
            FLowLink_TCP->HTTPS_count_upl       +=FLowLink_TCP->count_upl;
            FLowLink_TCP->HTTPS_count_downl       +=FLowLink_TCP->count_downl;

            FLowLink_TCP->HTTPS_count_upl_pkt       +=FLowLink_TCP->count_upl_pkt;
            FLowLink_TCP->HTTPS_count_downl_pkt       +=FLowLink_TCP->count_downl_pkt;
        }
    ...
    if(ipptr->ip_p == IPPROTO_ICMP){
        ICMP_HEADER *icmp;
        fprintf(fOutput, "----------------数据链路层 解析 ICMP -----------------\n");
        fprintf(fOutput,"ICMP -----\n");
        fprintf(fOutput,"type: %u\n", icmp->icmp_type);
        fprintf(fOutput,"sub code: %u\n", icmp->icmp_code);
    }else if(ipptr->ip_p == IPPROTO_UDP) {
        ...
        fprintf(fOutput, "校验和：%d\n", ntohs(udpptr->uh_sum));
        if((ntohs(udpptr->uh_sport)==53)||((udpptr->uh_dport)==53))
        {
            fprintf(fOutput, "              ----类型：DNS服务----                 \n\n");
            printf("\n第%d个网络数据包类型：（UDP）DNS服务\n", (id-1));
            FLowLink_UDP->DNS_count_conn++;
            FLowLink_UDP->DNS_count_upl         +=FLowLink_UDP->count_upl;
            FLowLink_UDP->DNS_count_downl       +=FLowLink_UDP->count_downl;

            FLowLink_UDP->DNS_count_upl_pkt       +=FLowLink_UDP->count_upl_pkt;
            FLowLink_UDP->DNS_count_downl_pkt       +=FLowLink_UDP->count_downl_pkt;
        }
    }
